...
def _download_index(
    ...
    if not year:
        logger.warning("No year mapping for period {}", period)
        return None
...
def find_steno_for_bod(
    ...
    if not day_refs:
        logger.debug(
            "Bod {} not found in steno index for period={} schuze={}",
            bod,
            period,
            schuze,
        )
    ...
    if not all_subpages:
        logger.debug(
            "No steno sub-pages found for period={} schuze={} bod={}",
            period,
            schuze,
            bod,
        )
    ...
    logger.debug(
        "Found {} steno sub-pages for period={} schuze={} bod={}",
        len(all_subpages),
        period,
        schuze,
        bod,
    ...
    for subpage_name in reversed(all_subpages):
